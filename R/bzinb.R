...
#' @param SE if \code{TRUE}, the standard error estimates are returned.
...
#' 
#' bzinb(xvec = data1[,1], yvec = data1[,2], 
#'       showFlag = F, SE = TRUE)
#' 
...
bzinb.base <- function (xvec, yvec, initial = NULL, tol = 1e-8, maxiter=50000, showFlag=FALSE,
  ...
  if (!SE & vcov) {warning("To get covariance matrix (vcov), SE must be TRUE. The covariance matrix will not be obtained.")}
  require(rootSolve)
